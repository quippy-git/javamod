...
public class SeekTable extends Metadata {
     ...
     */
    public SeekTable(final BitInputStream is, int length, final boolean isLast) throws IOException {
        super(isLast);
        final int numPoints = length / SEEKPOINT_LENGTH_BYTES;
     ...
     * @param isLast            True if this is the last Metadata block in the chain
     */
    public SeekTable(final SeekPoint[] points, final boolean isLast) {
        super(isLast);
        this.points = points;
    }
    ...
    public void write(final BitOutputStream os, final boolean isLast) throws IOException {
        ...
        for (final SeekPoint point : points)
		{
            point.write(os);
        }
     ...
     * @return  The selected seek point
     */
    public SeekPoint getSeekPoint(final int idx) {
