...
public class BitOutputStream {
    ...
    private boolean grow(final int minBlurbsToAdd) {
        final int newCapacity = Math.max(outCapacity * 2, outCapacity + minBlurbsToAdd);
        return resize(newCapacity);
    }

    private boolean ensureSize(final int bitsToAdd) {
    ...
    */
     ...
     */
    public boolean concatenateAligned(final BitOutputStream src) {
        final int bitsToAdd = src.totalBits - src.totalConsumedBits;
        if (bitsToAdd == 0) return true;
        if ((outBits != src.consumedBits) || !ensureSize(bitsToAdd)) return false;
        if (outBits == 0) {
     ...
     */
    public void resetReadCRC16(final short seed) {
        readCRC16 = seed;
    }
    ...
    public void writeZeroes(int bits) throws IOException {
        ...
        totalBits += bits;
        while (bits > 0) {
            final int n = Math.min(BITS_PER_BLURB - outBits, bits);
            buffer[outBlurbs] <<= n;
            bits -= n;
            outBits += n;
     ...
     * @throws IOException  On write error
     */
    public void writeRawUInt(final boolean val, final int bits) throws IOException {
    ...
    public void writeRawUInt(int val, int bits) throws IOException {
        ...
        if (((outCapacity << 3) < totalBits + bits) && !ensureSize(bits)) throw new IOException("Memory allocation error");
        ...
        totalBits += bits;
        while (bits > 0) {
            final int n = BITS_PER_BLURB - outBits;
            if (n == BITS_PER_BLURB) { // i.e. outBits == 0
                if (bits < BITS_PER_BLURB) {
                    ...
                    final int k = bits - BITS_PER_BLURB;
    ...
    public void writeRawInt(final int val, final int bits) throws IOException {
    ...
    public void writeRawUIntLittleEndian(final int val) throws IOException {
    ...
    public void writeByteBlock(final byte[] vals, final int nvals) throws IOException {
    ...
    public int riceBits(int val, final int parameter) {
        int msbs, uval;
    ...
    public void writeRiceSigned(int val, final int parameter) throws IOException {
        int totalBits;
        int interestingBits;
        int msbs;
        int uval;
    ...
    public void writeUTF8UInt(final int val) throws IOException {
        if (val < 0x80) {
            writeRawUInt(val, 8);
        } else if (val < 0x800) {
    ...
    public void writeUTF8ULong(final long val) throws IOException {
        if (val < 0x80) {
