...
class Mdct{
  ...
  void init(final int n){
    ...
    {
      final int mask=(1<<(log2n-1))-1;
      final int msb=1<<(log2n-2);
      for(int i=0; i<n/8; i++){
        int acc=0;
  ...
  synchronized void backward(final float[] in, final float[] out){
    ...
    final float[] xxx=mdct_kernel(x, w, n, n2, n4, n8);
    int xx=0;
...

    {
      ...
      for(int i=0; i<n4; i++){
        final float temp1=(xxx[xx]*trig[B+1]-xxx[xx+1]*trig[B]);
        final float temp2=-(xxx[xx]*trig[B]+xxx[xx+1]*trig[B+1]);
  ...
  private float[] mdct_kernel(float[] x, float[] w, final int n, final int n2, final int n4,
      final int n8){
    // step 2
...

    {
      for(int i=0; i<log2n-3; i++){
        ...
        for(int r=0; r<(k0>>>2); r++){
          int w1=wbase;
          w2=w1-(k0>>1);
          final float AEv=trig[A];
		float wA;
          final float AOv=trig[A+1];
		float wB;
          wbase-=2;
    ...
    // step 4, 5, 6, 7
    {
      ...
      for(int i=0; i<n8; i++){
        final int t1=bitrev[bit++];
        final int t2=bitrev[bit++];
        ...
        final float wACE=wA*trig[C];
        final float wBCE=wB*trig[C++];
        final float wACO=wA*trig[C];
        final float wBCO=wB*trig[C++];
