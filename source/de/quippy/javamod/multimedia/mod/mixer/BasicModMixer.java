...
//			}
	...
	public int getCurrentUsedChannels()
	{
		...
		for (int i=0; i<maxChannels; i++)
		{
			...
			if (aktMemo!=null && (aktMemo.isChannelActive() || aktMemo.hasMidiOutput())) result++; // can happen - is a race condition, that aktMemo becomes NULL
		}
		return result;
	}
	/**
...
//				samplesPerTick = ((sampleRate<<1) + (sampleRate>>1)) / currentBPM;
	 ...
	 * @since 28.06.2024 moved to the respective Mixers (ProTrackerMixher and ScreamTrackerMixer)
	 */
	protected abstract void setNewPlayerTuningFor(final ChannelMemory aktMemo, final int newPeriod);
//	{
...
//	}
	 ...
	 * @param aktMemo
	 */
	protected abstract void doKeyOff(final ChannelMemory aktMemo);
	/**
	 * @since 19.06.2020
	 * @param aktMemo
	 */
...
//		}
	 ...
	 * @param aktMemo
	 */
	protected void startRampDown(final ChannelMemory aktMemo)
	{
...
//				channelMemory[c].jumpLoopPatternRow = -1;
	 ...
	 * @param aktMemo
	 */
	protected void mixChannelIntoBuffers(final long[] leftBuffer, final long[] rightBuffer, final int startIndex, final int endIndex, final ChannelMemory aktMemo, final boolean isRampDown)
	{
		...
		final int doISPhere = (paulaFilter!=null)?0:(aktMemo.assignedInstrument!=null && aktMemo.assignedInstrument.resampling>-1)?aktMemo.assignedInstrument.resampling:doISP;
		...
		for (int i=startIndex; i<endIndex; i++)
		{
			...
			if (aktMemo.isForwardDirection)
			{
				...
				if (aktMemo.currentSamplePos >= loopEnd)
				{
					...
					{
						final int overShoot = aktMemo.currentSamplePos - loopEnd;
					    final int numberOfLoops = overShoot / loopLength;
					    final int rest = overShoot % loopLength;
			...
			else // Backwards in Ping Pong
			{
				...
				{
				    final int overShoot = loopStart - aktMemo.currentSamplePos;
				    final int numberOfLoops = overShoot / loopLength;
				    final int rest = overShoot % loopLength;
	...
	public int mixIntoBuffer(final long[] leftBuffer, final long[] rightBuffer, final int bufferSize)
	{
		...
		while (endIndex<bufferSize && !modFinished)
		{
			...
			for (int c=0; c<maxChannels; c++)
			{
				...
				if (rampDownMemo.actRampVolLeft<=0 && rampDownMemo.actRampVolRight<=0) rampDownMemo.instrumentFinished = true;
				if (rampDownMemo.currentTuning==0 && !rampDownMemo.instrumentFinished) rampDownMemo.currentTuning = 1;
				if (rampDownMemo.isChannelActive()) mixChannelIntoBuffers(leftBuffer, rightBuffer, startIndex, endIndex, rampDownMemo, true);
				...
				// Mix this channel?
				if (channelIsActive && !isPlayingMidi) mixChannelIntoBuffers(leftBuffer, rightBuffer, startIndex, endIndex, aktMemo, false);
